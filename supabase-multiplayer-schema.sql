-- Multiplayer Game Rooms and Related Tables
...
CREATE INDEX IF NOT EXISTS idx_game_rooms_active ON game_rooms(is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_game_rooms_dm ON game_rooms(dm_id);
...
-- Players can view other players in their rooms
CREATE POLICY "Players can view room players" ON room_players
    FOR SELECT USING (
        EXISTS (
...
-- Users can join rooms (if not full)
...
-- Players can view messages in their rooms
CREATE POLICY "Players can view room messages" ON room_messages
    FOR SELECT USING (
        EXISTS (
...
-- Players can send messages to their rooms
CREATE POLICY "Players can send messages" ON room_messages
    FOR INSERT WITH CHECK (
        auth.uid() = user_id AND
        EXISTS (
            SELECT 1 FROM room_players rp 
